...
schemaByType.set(SchemaKind.InvitationGetResponse, {
    content: invitationGetResponseSchema,
    dependencies: [commonDefinitionsSchema],
})
schemaByType.set(SchemaKind.InvitationPostResponse, {
    content: invitationPostResponseSchema,
    dependencies: [commonDefinitionsSchema],
})
schemaByType.set(SchemaKind.InvitationsGetResponse, {
    content: invitationsGetResponseSchema,
    dependencies: [commonDefinitionsSchema],
})
schemaByType.set(SchemaKind.MessagesGetResponse, {
    content: messagesGetResponseSchema,
    dependencies: [],
})
...
export const validateSchemaAsync = async <TResponse>(
    ...
    return responsePromise.then((response) => {
        ...
        if (!valid) {
            console.error(
                ...
                JSON.stringify(responseSchema.content)
            )
        }
        return response
    })
