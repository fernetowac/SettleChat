export class Ascending {
    ...
    public static by = <T, TProperty extends number | Date | string>(getProperty: ((value: T) => TProperty)) =>
        (first: T, second: T) =>
...
export class Descending {
    public static by = <T, TProperty extends number | Date | string>(getProperty: ((value: T) => TProperty)) =>
        (first: T, second: T) =>
            Ascending.by(getProperty)(second, first)//Note switched values when calling ascending comparer
}
 ...
 */
export const lowestBy = <T, TSortProperty>(collection: T[], getSortProperty: ((item: T) => TSortProperty)): T | undefined =>
    collection.length ?
        collection.reduce((previous, current) => getSortProperty(previous) > getSortProperty(current) ? current : previous) :
        undefined
 ...
 * @param getSortProperty Specifies property of collection item to be sorted by
 */
export const getLowestBy = <T, TSortProperty>(getSortProperty: ((item: T) => TSortProperty)): ((collection: T[]) => T | undefined) =>
 ...
 * @param getSortProperty Specifies property of collection item to be sorted by
 */
export const highestBy = <T, TSortProperty>(collection: T[], getSortProperty: ((item: T) => TSortProperty)): T | undefined =>
    collection.length ?
        collection.reduce((previous, current) => getSortProperty(previous) < getSortProperty(current) ? current : previous) :
        undefined
 ...
 * @param getSortProperty Specifies property of collection item to be sorted by
 */
export const getHighestBy = <T, TSortProperty>(getSortProperty: ((item: T) => TSortProperty)): ((collection: T[]) => T | undefined) =>
    (collection: T[]): T | undefined =>
