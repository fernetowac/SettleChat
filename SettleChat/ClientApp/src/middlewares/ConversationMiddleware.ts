...
export const createConversationMiddleware = <
    ...
    D extends Dispatch & ThunkDispatch<ApplicationState, {}, AnyAction>
>(): Middleware<{}, S, D> => {
    const dispatchIdentityStatusChangeIfNeeded = <TApi extends MiddlewareAPI<D, S>>(
        api: TApi,
        status: UserStatus
    ) => {
    ...
    const dispatchRequestConversationWithUsersAndItsLastMessage = <
        TApi extends MiddlewareAPI<D, S>
    >(
        api: TApi,
    ...
    return (api) => {
        return (next) => (action) => {
            ...
            if (signalRActions.connectionEstablished.match(action)) {
                ...
                if (
                    action.payload.conversationUser.userId === state.identity.userId &&
                    !state.conversation.detail.ids.includes(conversationId)
                ) {
