...
export const createIdentityMiddleware = <S extends ApplicationState>(): Middleware<{}, S>/*: Middleware<any, ApplicationState, AppDispatch>*/ => {
    const populateState = (userProfile: Profile | null, dispatch: Dispatch<AnyAction>, getState: () => S): void => {
        const isAuthenticated = !!userProfile;
        const storeUserId = getState().identity.userId;
        if ((!userProfile && !storeUserId) || (userProfile && userProfile.sub === storeUserId)) {
            return;
        }
        dispatch(identityChangedActionCreator({
