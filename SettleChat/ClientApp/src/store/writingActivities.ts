...
export const updateWritingActivity = createAsyncThunk<
    void,
    { conversationId: string; writingActivity: WritingActivityData },
    AppThunkApiConfig
>('writingActivity/update', async ({ conversationId, writingActivity }, thunkAPI) => {
    ...
    return await fetchPut<void>(
        `/api/conversations/${conversationId}/writingactivity`,
        writingActivity
    )
})
    ...
    reducers: {
        received: {
            reducer: (state, action: PayloadAction<ReceivedWritingActivityStateItem>) => {
                const itemOfUserConversation = state.find(
                    (x) =>
                ...
                if (
                    !itemOfUserConversation ||
                    (itemOfUserConversation &&
                        itemOfUserConversation.lastChangeClientUnixTimeInMs <
                            action.payload.lastChangeClientUnixTimeInMs)
                ) {
                    const itemsOfOtherUsers = state.filter(
                        (x) =>
                            x.userId !== action.payload.userId ||
                            x.conversationId !== action.payload.conversationId
                    )
...
export const {
    actions: writingActivitiesActions,
    reducer: writingActivitiesReducer,
} = writingActivitiesSlice
