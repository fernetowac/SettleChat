...
export const { selectAll: selectAllConversationUsers } = conversationUsersAdapter.getSelectors(
    (state: ApplicationState) => state.conversation.conversationUsers
)
export const conversationUsersByConversationIdSelector = (
    state: ApplicationState,
    { conversationId }: { conversationId: string }
) =>
    selectAllConversationUsers(state).filter(
        (conversationUser) => conversationUser.conversationId === conversationId
    )
export function conversationUserByIdsSelector(
    state: ApplicationState,
    { userId, conversationId }: { userId: string; conversationId: string }
) {
    return selectAllConversationUsers(state).find(
        (conversationUser) =>
            conversationUser.userId === userId && conversationUser.conversationId === conversationId
    )
}
...
const conversationUsersSlice = createSlice({
    ...
    extraReducers: (builder) => {
        builder
            .addCase(requestConversationUsers.fulfilled, (state, action) =>
                conversationUsersAdapter.upsertMany(state, action.payload.conversationUsers)
            )
            .addCase(conversationUserAdded, (state, action) =>
                conversationUsersAdapter.addOne(state, action.payload.conversationUser)
            )
            ...
            .addCase(identityChangedActionCreator, conversationUsersAdapter.getInitialState)
    },
})
