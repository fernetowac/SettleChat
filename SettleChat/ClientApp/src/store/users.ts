...
export const { selectById: selectUserById } = usersAdapter.getSelectors(
    (state: ApplicationState) => state.conversation.users
)
export const userByIdSelector = usersAdapter.getSelectors<ApplicationState>(
    (state) => state.conversation.users
).selectById
export const allUsersSelector = usersAdapter.getSelectors<ApplicationState>(
    (state) => state.conversation.users
).selectAll
...
const usersSlice = createSlice({
    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase(identityChangedActionCreator, usersAdapter.getInitialState)
    },
})
