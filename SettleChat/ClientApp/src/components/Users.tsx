        ...
        <span>
            ...
            {parts.map((part, i) => (
                <span
                    key={i}
                    style={
                        part.toLowerCase() === highlight.toLowerCase() ? { fontWeight: 'bold' } : {}
                    }
                >
                    {part}
                </span>
                ...
                {users.map((user, index) => {
                    ...
                    const highlightedName =
                        userNameFilter && userNameFilter.length > 0
                            ? getHighlightedText(name, userNameFilter)
                            : name
                    return (
                        <React.Fragment key={user.id}>
                            <ListItem alignItems="flex-start" key={user.id}>
                                ...
                                {listItemSecondaryActionContent && (
                                    <ListItemSecondaryAction
                                        onClick={() =>
                                            listItemSecondaryActionHandler &&
                                            listItemSecondaryActionHandler(user.id, name)
                                        }
                                    >
                                        {listItemSecondaryActionContent}
                                    </ListItemSecondaryAction>
                            ...
                            {index < users.length - 1 ? (
                                <Divider component="li" key={`${user.id}_divider`} />
                            ) : (
                                ''
                            )}
                        </React.Fragment>
...
const getFilteredUsersIds = (
    ...
    const usersIdsFilteredByName = users
        .filter((user) => user.userName.toUpperCase().includes(upperCasedNameFilter))
        .map((user) => user.id)
    const conversationUsersUserIdsFilteredByNickname = conversationUsers
        .filter(
            (conversationUser) =>
                conversationUser.nickname &&
                conversationUser.nickname.toUpperCase().includes(upperCasedNameFilter)
        )
...
const mapStateToProps = (
    ...
    let users = conversationId
        ? selectUsersByConversationId(state, { conversationId })
        : allUsersSelector(state)
    let conversationUsers = conversationId
        ? conversationUsersByConversationIdSelector(state, { conversationId })
        : selectAllConversationUsers(state)
    ...
    if (userNameFilter !== undefined && userNameFilter.length > 0) {
        ...
        conversationUsers = conversationUsers.filter((conversationUser) =>
            filteredUsersIds.includes(conversationUser.userId)
        )
    }
