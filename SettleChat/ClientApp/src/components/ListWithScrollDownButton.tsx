...
const scrollUpTrigger = (store: React.MutableRefObject<any>, options: { disableHysteresis?: boolean, threshold?: number, target: HTMLElement }) => {
...
export const ListWithScrollDownButton = React.forwardRef<HTMLUListElement, ListProps>((props, forwardedRef) => {
    const listBottomRef = React.useRef<HTMLLIElement>(null);
    let listRef = forwardedRef ? forwardedRef as React.RefObject<HTMLUListElement> : React.useRef<HTMLUListElement>(null);
        ...
        <React.Fragment>
            ...
            <Box height={0} display="flex" position="relative" justifyContent="center">
                <Zoom in={isScrolledUp}>
                    <Box onClick={onScrollTopClick} component="span" role="presentation" className={scrollTopClasses.root}>
