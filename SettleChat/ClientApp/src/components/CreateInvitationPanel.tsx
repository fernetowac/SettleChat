...
type CreateInvitationPanelProps = ReturnType<typeof mapStateToProps> &
...
function CreateInvitationPanel(props: CreateInvitationPanelProps) {
    ...
    const buildTokenLink = (token: string): string =>
        `${window.location.origin}/invitation/${token}`

    React.useEffect(() => {
        let isMounted = true
        requestInvitations(conversationId).finally(() => {
            if (isMounted) {
                setIsLoading(false)
            }
        })
        ...
        <div>
            <FormControl component="fieldset">
                <FormControlLabel
                    ...
                    label="Unconstrained"
                />
                <Button color="primary" variant="contained" onClick={handleAddInvitationSubmit}>
                    Create invitation
                </Button>
            </FormControl>
            <Box>
                                ...
                                <React.Fragment key={invitation.id}>
                                    <ListItem key={invitation.id} dense>
                                        ...
                                        <ListItemSecondaryAction>
                                            <IconButton
                                                edge="end"
                                                onClick={() => handleOnCopyLink(invitationLink)}
                                            >
                                                <FileCopyIcon />
                                            </IconButton>
                                        </ListItemSecondaryAction>
...
const getInvitations = (state: ApplicationState, conversationId: string): Invitation[] =>
    state.conversation.invitations.filter(
        (invitation) => invitation.conversationId === conversationId
    )

const getSortedInvitations = (invitations: Invitation[]): Invitation[] =>
    [...invitations].sort(Ascending.by((x) => x.created))
...
const mapDispatchToProps = (dispatch: AppDispatch) => ({
    actions: {
        requestInvitations: (conversationId: string) =>
            dispatch(requestInvitations(conversationId)),
        createInvitation: (newInvitation: NewInvitation) =>
            dispatch(createInvitation(newInvitation)),
        addNotification: (message: NotificationAddActionInput | string) =>
...
export default connect(mapStateToProps, mapDispatchToProps)(CreateInvitationPanel as any)
