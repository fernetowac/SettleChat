...
const Conversations = (props: ConnectedProps<typeof connector>) => {
    ...
    const {
        ...
        userId,
        isAuthenticated,
        hasMultipleUsersByConversationId,
    } = props
    React.useEffect(() => {
                    ...
                    return (
                        <React.Fragment key={conversation.id}>
                            ...
                            >
                                <ListItemAvatar>
                                    <Avatar>
                                        {hasMultipleUsersByConversationId.get(conversation.id) ===
                                        false ? (
                                            <PersonIcon />
                                        ) : (
                                            <GroupIcon />
                                ...
                                <ListItemText
                                    ...
                                    secondary={
                                        <>
                                            ...
                                            <TimeAgo
                                                date={conversation.lastActivityTimestamp}
                                                minPeriod={60}
                            ...
                            {index < props.conversations.length - 1 ? (
                                <Divider component="li" key={`${conversation.id}_divider`} />
                            ) : (
                                ''
                            )}
                        </React.Fragment>
 ...
 */
const getMapOfHasMultipleOtherUsersByConversationId = (
    conversationUsers: ConversationUserMeta[],
    currentUserId: string | null
) =>
    arrayToMap(
        [...groupBy(conversationUsers, (item) => item.conversationId)].map(
            (conversationUsersByConversationId) => ({
                conversationId: conversationUsersByConversationId[0],
                hasMultipleUsers:
                    conversationUsersByConversationId[1].filter((x) => x.userId !== currentUserId)
                        .length > 1,
            })
        ),
