...
export const createInvitation = createAsyncThunk(
    'invitations/create',
    async (newInvitation: NewInvitation) => {
        const url = `/api/conversations/${newInvitation.conversationId}/invitations`
        const payload = {
            isPermanent: newInvitation.isPermanent,
        }
        return await fetchPost<InvitationResponse>(
            url,
            payload,
            true,
            SchemaKind.InvitationPostResponse
        )
    }
)
 ...
 */
export const requestInvitations = createAsyncThunk(
    'invitations/requestList',
